...
export default function AddCar() {
  ...
  const handleAiParse = async () => {
    if (!aiPrompt.trim()) {
      toast.error('Please paste car details or WhatsApp message first');
      return;
    }

    setIsAiLoading(true);
    const toastId = toast.loading('AI is analyzing car details & automotive specs...');

    try {
      const res = await axiosInstance.post('/ai/parse-car', { text: aiPrompt });
      if (res.data.success && res.data.data) {
        const p = res.data.data;

        // Populate Core Details
        if (p.make) setValue('make', p.make);
        if (p.model) setValue('model', p.model);
        if (p.variant) setValue('variant', p.variant);
        if (p.manufacturingYear) setValue('manufacturingYear', String(p.manufacturingYear));
        if (p.registerYear) setValue('registerYear', String(p.registerYear));
        if (p.price) setValue('price', String(p.price));
        ...
        if (res.data.source === 'gemini') {
          toast.success('Car details & specifications auto-filled with Gemini AI! ✨', { id: toastId });
        } else {
          toast.success('Car details auto-filled via Smart Pattern Parser! ⚡', { id: toastId });
        }
      }
    } catch (err) {
      console.error('AI parse error:', err);
      toast.error(err.response?.data?.message || 'Failed to auto-fill car details', { id: toastId });
    } finally {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="pb-24">
      ...
      <div className="mb-8">
        ...
        <p className="font-body text-sm text-text-muted mt-1">
          Fill in the details below or use AI Quick-Fill to instantly list a new car in the inventory.
        </p>
      </div>

      {/* ── AI Quick-Fill / WhatsApp Parser Card ── */}
      <div className="mb-8 rounded-2xl bg-surface border border-brand-orange/20 p-4 sm:p-6 shadow-md shadow-brand-orange/5 relative overflow-hidden">
        {/* Ambient Warm Brand Glow */}
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-brand-orange to-amber-500 flex items-center justify-center text-white shadow-md shadow-brand-orange/25">
                <Sparkles className="w-5 h-5" />
              </div>
              <div>
                <h3 className="font-heading font-bold text-base text-text flex items-center gap-2">
                  AI Quick-Fill & WhatsApp Parser
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-brand-orange/10 text-brand-orange border border-brand-orange/25">
                    Gemini AI Powered
                  </span>
                </h3>
                <p className="font-body text-xs text-text-muted mt-0.5">
                  Paste raw WhatsApp dealer message or car text — AI fills Make, Model, Year, Price, Specs & Features automatically.
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="text-xs font-semibold text-brand-orange hover:text-accent-hover transition-colors"
            >
            ...
            <div className="space-y-3 pt-2">
              <div className="relative">
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 bg-background rounded-xl border border-gray-200 font-mono text-xs text-text placeholder:text-text-muted/40 outline-none transition-all focus:border-brand-orange/50 focus:ring-2 focus:ring-brand-orange/10 resize-none"
                />
                {aiPrompt && (
                  <button
                    type="button"
                    onClick={() => setAiPrompt('')}
                    className="absolute top-2.5 right-2.5 text-xs text-text-muted hover:text-text px-2 py-1 rounded bg-surface/80 border border-gray-100"
                  >
                    Clear
                  </button>
                )}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1">
                <div className="flex items-center gap-2 flex-wrap">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 rounded-lg bg-surface border border-gray-200 text-text font-body text-xs font-semibold hover:bg-gray-50 transition-colors min-h-[38px] active:scale-95"
                  >
                    <Clipboard className="w-3.5 h-3.5 text-brand-orange" />
                    Paste Clipboard
                  </button>
                  <span className="text-[11px] text-text-muted">
                    Supports WhatsApp bold text & dealer codes
                  </span>
                </div>
                ...
                <button
                  ...
                  className="w-full sm:w-auto justify-center inline-flex items-center gap-2 px-5 py-2.5 min-h-[42px] rounded-xl bg-gradient-to-r from-brand-orange to-amber-500 hover:from-accent-hover hover:to-amber-600 text-white font-body text-xs font-bold shadow-md shadow-brand-orange/25 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[0.98]"
                >
                  {isAiLoading ? (
                    <>
