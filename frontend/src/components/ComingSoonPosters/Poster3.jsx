...
function NeonStat({ label, value, icon }) {
  return (
    <div className="flex items-center gap-2 md:gap-3 lg:gap-1.5 bg-black/60 border border-fuchsia-600/30 p-2 md:p-2.5 lg:p-1.5 backdrop-blur-md group hover:bg-[#1a0b2e]/80 transition-colors duration-300 relative overflow-hidden transform skew-x-0 md:-skew-x-[12deg] rounded md:rounded-none">
      {/* Glitch hover effect */}
      ...
      <div className="text-fuchsia-400 group-hover:text-cyan-300 w-8 h-8 lg:w-6 lg:h-6 flex items-center justify-center shrink-0 skew-x-0 md:skew-x-[12deg] transition-colors drop-shadow-[0_0_8px_currentColor]">
        {icon}
      </div>
      <div className="flex flex-col skew-x-0 md:skew-x-[12deg] w-[calc(100%-2rem)]">
        <span className="text-fuchsia-300/80 text-[8px] lg:text-[9px] font-black uppercase tracking-[0.2em] mb-0.5">{label}</span>
        <span className="text-white text-xs md:text-sm lg:text-xs font-bold uppercase truncate drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]">{value}</span>
      </div>
...
export default function Poster3({ car }) {
  ...
  return (
    <div className="w-full relative shadow-[0_0_50px_rgba(192,38,211,0.3)] rounded-[2rem] overflow-hidden bg-[#0A0510] border border-fuchsia-900/40 p-1.5 min-h-[260px] lg:min-h-[150px] group/poster">
      ...
      <div className="w-full h-full bg-gradient-to-b from-[#130724] to-[#040108] rounded-[1.6rem] relative overflow-hidden pb-6">
        ...
        <div className="relative z-20 flex flex-col h-full p-3 lg:p-6 mt-1">
          ...
          {/* Header */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 md:gap-0 mb-4 lg:mb-2">
            <div className="flex items-center gap-2 bg-black/40 px-3 py-1.5 lg:py-1 border border-fuchsia-500/30 rounded backdrop-blur text-fuchsia-400 font-mono text-[10px] tracking-widest uppercase">
              <Terminal className="w-4 h-4 lg:w-3 lg:h-3 animate-pulse" />
              SYS_OVERRIDE_ACTIVE
            </div>

            {/* 3D Drop Shadow Glitch Title */}
            <div className="relative group/title z-30 transform hover:scale-105 transition-transform">
              <h2 className="font-heading text-2xl md:text-3xl lg:text-2xl font-black italic uppercase tracking-[0.1em] text-white absolute top-[2px] left-[2px] text-cyan-400 opacity-70 mix-blend-screen pointer-events-none">
                COMING_SOON
              </h2>
              ...
              <h2 className="relative font-heading text-2xl md:text-3xl lg:text-2xl font-black italic uppercase tracking-[0.1em] text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]">
                COMING_SOON
              </h2>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col md:flex-row gap-4 items-center justify-center mt-1">
            ...
            {/* Left: Car Viewer */}
            <div className="flex-1 w-full relative flex items-center justify-center min-h-[150px] md:min-h-[180px] lg:min-h-[140px] z-20">
              {car.image ? (
                <div className="relative w-full max-w-2xl group/img cursor-pointer">
                  ...
                  <img src={car.image} alt={car.title} className="w-full h-full object-contain max-h-[220px] lg:max-h-[180px] relative z-10 drop-shadow-[0_20px_40px_rgba(0,0,0,0.9)] transform group-hover/img:-translate-y-4 transition-transform duration-700 ease-out" />
            ...
            {/* Right: Data Deck */}
            <div className="w-full md:w-[280px] flex flex-col gap-3 lg:gap-1.5 shrink-0 relative z-30">
              <div className="bg-black/50 border border-fuchsia-500/40 p-3 md:p-4 lg:p-2.5 transform skew-x-0 md:-skew-x-[12deg] mb-2 lg:mb-0 shadow-[0_0_20px_rgba(217,70,239,0.15)] relative overflow-hidden backdrop-blur-md rounded md:rounded-none">
                <div className="absolute top-0 right-0 w-8 h-full bg-cyan-400/10 skew-x-[20deg] translate-x-4"></div>
                <h3 className="text-fuchsia-400 font-mono text-xs lg:text-[10px] uppercase tracking-[0.2em] mb-1 lg:mb-0.5 skew-x-0 md:skew-x-[12deg]">Ident:</h3>
                <p className="text-white font-heading text-xl md:text-2xl lg:text-lg font-black italic tracking-wider skew-x-0 md:skew-x-[12deg] drop-shadow-[0_0_5px_rgba(255,255,255,0.4)] truncate">
                  {car.make} <span className="text-cyan-300">{car.model}</span>
                </p>
              </div>

              <div className="flex flex-col gap-2 lg:gap-1">
                <NeonStat icon={<Fuel className="w-4 h-4 lg:w-3 lg:h-3" />} label="PWR/DRV" value={`${car.fuelType} // ${car.transmission}`} />
                <NeonStat icon={<CarIcon className="w-4 h-4 lg:w-3 lg:h-3" />} label="FORM.F" value={car.bodyType || 'SUV'} />
                <NeonStat icon={<Palette className="w-4 h-4 lg:w-3 lg:h-3" />} label="CHROMA" value={car.color || 'DFT'} />
                <NeonStat icon={<Gauge className="w-4 h-4 lg:w-3 lg:h-3" />} label="DST.LOG" value={`${car.kms?.toLocaleString() || 0}U`} />
              </div>

              {/* Notify Me Button */}
              <button className="w-full lg:mt-1 py-3 lg:py-2 bg-fuchsia-600 hover:bg-fuchsia-500 text-white lg:text-sm font-black italic tracking-[0.2em] uppercase rounded-none skew-x-0 md:-skew-x-[12deg] shadow-[0_0_20px_rgba(217,70,239,0.5)] transition-all duration-300 transform hover:scale-[1.02] flex items-center justify-center gap-2 border border-fuchsia-400">
                <span className="skew-x-0 md:skew-x-[12deg] flex items-center gap-2"><Zap className="w-5 h-5 lg:w-4 lg:h-4 animate-pulse" /> Notify Me</span>
              </button>
            </div>
