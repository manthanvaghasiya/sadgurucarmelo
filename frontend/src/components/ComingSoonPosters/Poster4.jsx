...
export default function Poster4({ car }) {
  ...
  return (
    <div className="w-full relative shadow-2xl rounded-tr-[5rem] rounded-bl-[5rem] overflow-hidden bg-[#0d0d0d] border-t-2 border-r-2 border-red-600 min-h-[260px] lg:min-h-[280px] group/poster">
      ...
      <div className="relative z-10 flex flex-col p-4 md:p-6 h-full">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 relative">
           ...
           <div className="z-10 pl-6 md:pl-16 pt-4">
              ...
              <h2 className="font-heading text-2xl md:text-4xl font-black text-white uppercase tracking-tighter transform skew-x-0 md:-skew-x-[10deg] drop-shadow-lg flex flex-wrap items-center gap-2">
                 ...
                 {/* High-Impact Badge */}
                 <div className="ml-2 mt-2 md:mt-0 px-3 py-1 bg-red-600 text-white text-[10px] md:text-sm tracking-[0.2em] transform skew-x-0 md:-skew-x-[10deg] shadow-[4px_4px_0_#000] border-l-2 border-white">
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-4 md:gap-8 mt-2 items-center">
          ...
          {/* Main Visual Display */}
          <div className="flex-1 relative w-full min-h-[150px] md:min-h-[220px] flex items-center justify-center z-20 group/car">
             ...
             {car.image ? (
                <img src={car.image} className="w-[95%] h-[95%] object-contain relative z-10 drop-shadow-[0_20px_30px_rgba(0,0,0,0.9)] transform group-hover/car:scale-[1.05] group-hover/car:-translate-y-2 transition-transform duration-[1s] ease-out" alt={car.title}/>
             ) : (
                <div className="w-[85%] md:w-[80%] h-[80%] border border-dashed border-red-600/40 bg-black/50 backdrop-blur rounded flex flex-col items-center justify-center z-10 transform skew-x-0 md:-skew-x-[5deg] px-2 text-center">
          ...
          <div className="w-full lg:w-[350px] shrink-0 grid grid-cols-2 md:grid-cols-2 lg:grid-cols-1 gap-2 md:gap-4 z-30">
...
function RaceBlock({icon, label, value}) {
  return (
    <div className="bg-[#111] border-l-[4px] md:border-l-[6px] border-red-600 py-2 md:py-3 px-3 md:px-4 transform skew-x-0 md:-skew-x-[15deg] flex items-center justify-between group hover:bg-[#1a1a1a] hover:border-red-500 transition-colors shadow-[4px_4px_15px_rgba(0,0,0,0.5)]">
      <div className="flex flex-col transform skew-x-0 md:skew-x-[15deg] w-[calc(100%-1.5rem)]">
        <span className="text-stone-500 text-[9px] font-black uppercase tracking-[0.2em] mb-0.5">{label}</span>
        <span className="text-white text-xs md:text-sm lg:text-base font-black uppercase truncate group-hover:text-red-50 transition-colors">{value}</span>
      </div>
