...
function EcoSpec({ icon, label, value }) {
  return (
    <div className="flex flex-col md:flex-row items-center md:items-start gap-2 md:gap-4 p-2 md:p-4 lg:p-5 bg-white/40 backdrop-blur-xl rounded-2xl border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_15px_40px_rgba(20,83,45,0.08)] hover:bg-white/60 transition-all duration-500 group relative overflow-hidden">
      {/* Animated shine line */}
      ...
      <div className="text-emerald-600 bg-gradient-to-br from-emerald-50 to-green-100 w-8 h-8 md:w-12 md:h-12 rounded-full flex items-center justify-center shrink-0 border border-white shadow-inner group-hover:scale-110 transition-transform duration-500">
        {icon}
      </div>
      <div className="flex flex-col items-center md:items-start overflow-hidden w-full">
        <span className="text-emerald-800/50 text-[9px] lg:text-[10px] font-bold uppercase tracking-[0.2em] mb-1">{label}</span>
        <span className="text-emerald-950 font-black text-xs md:text-sm lg:text-base uppercase tracking-wider truncate w-full text-center md:text-left drop-shadow-[0_1px_10px_rgba(255,255,255,1)]">{value}</span>
      </div>
      <style jsx>{`
         @keyframes shine {
           100% { left: 200%; }
...
export default function Poster5({ car }) {
  ...
  return (
    <div className="w-full relative shadow-[0_40px_100px_rgba(16,185,129,0.15)] rounded-[3rem] overflow-hidden bg-[#f0fdf4] border border-white min-h-[200px] lg:min-h-[280px] group/poster">
      ...
      <div className="relative z-10 flex flex-col p-4 md:p-6 h-full justify-between gap-4 py-4 md:py-6">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-6 z-20">
          ...
          <div className="flex flex-col items-center md:items-end">
            <h2 className="font-heading text-2xl md:text-4xl lg:text-5xl font-light text-emerald-950 tracking-[0.1em] md:tracking-[0.2em] uppercase flex items-center gap-2 md:gap-4 drop-shadow-sm">
        ...
        <div className="flex-1 w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 relative mt-4">
          ...
          <div className="relative w-full md:w-[55%] lg:w-[60%] min-h-[160px] md:min-h-[220px] flex items-center justify-center z-10">
            ...
            {car.image ? (
              <img src={car.image} alt={car.title} className="w-[110%] h-auto max-h-full object-contain drop-shadow-[0_40px_50px_rgba(0,0,0,0.2)] hover:scale-105 hover:-translate-y-4 transition-all duration-[1s] ease-out relative z-10" />
            ) : (
              <div className="w-[85%] md:w-[70%] h-[70%] border border-dashed border-emerald-300/50 rounded-[2rem] bg-emerald-50/30 backdrop-blur-lg flex flex-col items-center justify-center text-emerald-800/40 font-heading font-medium tracking-[0.1em] md:tracking-[0.2em] relative z-10 shadow-[inset_0_0_30px_rgba(255,255,255,0.8)] gap-2 md:gap-4 px-4 text-center">
                <ShieldCheck className="w-8 h-8 md:w-12 md:h-12 opacity-50" />
                <span className="text-sm md:text-base">REVEAL PENDING</span>
              </div>
            )}

            {/* Minimalist Title Floating */}
            <div className="absolute -bottom-8 md:-bottom-12 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-xl px-4 md:px-8 py-2 md:py-3.5 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.05)] border border-white z-20 w-[95%] max-w-max text-center overflow-hidden">
              <span className="font-heading font-light text-sm md:text-xl lg:text-2xl text-emerald-950 tracking-[0.1em] md:tracking-[0.3em] uppercase block truncate w-full">
          ...
          <div className="w-full md:w-[45%] lg:w-[40%] grid grid-cols-2 lg:grid-cols-2 gap-3 lg:gap-5 z-20 pb-4 md:pb-0">
